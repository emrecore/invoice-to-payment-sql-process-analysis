...
-- Shows all process events in the correct order.
-- COMMENT: event_id is included as a deterministic tie-breaker
...
-- ============================================================
-- 4. Calculate duration between consecutive completed events
-- Measures completed step duration in hours.
-- COMMENT: Pending and rejected events are excluded so that
-- incomplete work does not distort duration measurements.
-- ============================================================
...
-- ============================================================
-- 5. Completed process step duration with department information
-- Adds responsible department for the current completed event.
-- ============================================================
...
ORDER BY
    event_steps.invoice_id,
    event_steps.event_timestamp,
    event_steps.event_id;

-- ============================================================
-- 6. Average duration by completed process transition
...
-- ============================================================
-- 8. Average completed process duration by invoice
-- Measures time from first completed event to last completed event.
-- ============================================================
...
SELECT
    ...
    v.vendor_name,
    i.invoice_status,
    MIN(ie.event_timestamp) AS first_completed_event_timestamp,
    MAX(ie.event_timestamp) AS last_completed_event_timestamp,
    TIMESTAMPDIFF(
        HOUR,
        MIN(ie.event_timestamp),
        MAX(ie.event_timestamp)
    ) AS total_process_duration_hours,
    ROUND(
        TIMESTAMPDIFF(
            HOUR,
            MIN(ie.event_timestamp),
        ...
        2
    ) AS total_process_duration_days
FROM invoices i
...
-- Measures completed processing duration by process type.
-- ============================================================
...
WITH invoice_cycle_times AS (
    SELECT
        i.invoice_id,
        CASE
            WHEN i.exception_type_id IS NULL
...
SELECT
    ...
    ROUND(
        AVG(total_process_duration_hours),
        2
    ) AS avg_process_duration_hours,
    ROUND(
        AVG(total_process_duration_hours) / 24,
        2
    ) AS avg_process_duration_days,
...
-- Identifies vendors linked to longer completed processing times.
-- ============================================================
...
SELECT
    ...
    ROUND(
        AVG(ict.total_process_duration_hours),
        2
    ) AS avg_process_duration_hours,
    ROUND(
        AVG(ict.total_process_duration_hours) / 24,
        2
    ) AS avg_process_duration_days,
...
-- Measures time from the previous completed event to Approval.
-- ============================================================
...
WITH exception_events AS (
    SELECT
        ...
        MIN(
            CASE
                WHEN event_name = 'Exception Raised'
                    THEN event_timestamp
            END
        ) AS exception_raised_timestamp,
        MIN(
            CASE
                WHEN event_name IN (
                    'Correction Received',
...
exception_durations AS (
    SELECT
        ...
        TIMESTAMPDIFF(
            HOUR,
            ee.exception_raised_timestamp,
            ee.correction_timestamp
        ) AS exception_resolution_hours
    FROM exception_events ee
    INNER JOIN invoices i
        ON ee.invoice_id = i.invoice_id
...
SELECT
    ...
    ROUND(
        AVG(ed.exception_resolution_hours),
        2
    ) AS avg_exception_resolution_hours,
    ROUND(
        AVG(ed.exception_resolution_hours) / 24,
        2
    ) AS avg_exception_resolution_days,
...
-- Shows invoices where the latest recorded event is not Paid.
-- COMMENT: All events are included because this query identifies
-- the current state of open work, including pending events.
-- ============================================================
...
-- COMMENT: All events are retained because variants describe the
-- full observed process path, including current pending activity.
-- ============================================================
...
-- Combines full variants with completed process duration.
-- ============================================================
...
GROUP BY pv.process_variant
ORDER BY
    avg_process_duration_hours DESC;

-- ============================================================
-- 22. Bottleneck analysis by department
-- Uses the department responsible for the current completed event.
-- ============================================================
...
-- ============================================================
-- 23. Top 10 slowest individual completed process steps
-- Identifies extreme completed cases.
-- ============================================================
...
WITH receipt_to_schedule AS (
    SELECT
        ...
        MIN(
            CASE
                WHEN event_name = 'Invoice Received'
                    THEN event_timestamp
            END
        ) AS invoice_received_timestamp,
        MAX(
            CASE
                WHEN event_name = 'Payment Scheduled'
                    THEN event_timestamp
            END
        ) AS payment_scheduled_timestamp
    FROM invoice_events
...
SELECT
    ...
    CASE
        WHEN payment_scheduled_timestamp IS NULL
...
FROM sla_classification
GROUP BY processing_sla_status
ORDER BY
    invoice_count DESC;
...
SELECT
    ...
    SUM(
        CASE
            WHEN sc.processing_sla_status = 'Within SLA'
                THEN 1
            ELSE 0
        END
    ) AS within_sla_count,
    SUM(
        CASE
            WHEN sc.processing_sla_status = 'SLA Breach'
                THEN 1
            ELSE 0
        END
    ) AS sla_breach_count,
    SUM(
        CASE
            WHEN sc.processing_sla_status = 'Not Scheduled Yet'
                THEN 1
            ELSE 0
        END
...
-- Compact KPI view of completed process performance.
-- ============================================================
...
SELECT
    COUNT(*) AS invoices_with_completed_events,
    ROUND(
        AVG(ict.total_process_duration_hours),
        2
    ) AS avg_total_process_duration_hours,
    ROUND(
        AVG(ict.total_process_duration_hours) / 24,
        2
    ) AS avg_total_process_duration_days,
    SUM(
        CASE
            WHEN pc.process_type = 'Exception Process'
                THEN 1
            ELSE 0
        END
    ) AS exception_process_count,
    SUM(
        CASE
            WHEN pc.process_type = 'Standard Process'
                THEN 1
            ELSE 0
        END
    ) AS standard_process_count
FROM invoice_cycle_times ict
